...
public class movieDetail extends HttpServlet {
    ...
    throws ServletException, IOException {
       ...
       CategoriesDAO cdao = new CategoriesDAO();
        ...
        listc = cdao.getAll();
        c=comment.getComment(id);
       Movies m = new Movies();
       m = dao.getMovieByID(id);
       request.setAttribute("movie", m);
       request.setAttribute("listc", listc);
       request.setAttribute("c", c);
       request.getRequestDispatcher("../view/movieDetail.jsp").forward(request, response);
    } 
    ...
    throws ServletException, IOException {
        ...
        if(request.getParameter("comment")!=null){
            ...
            cmt.addCMT(id,name,comment,date);
            doGet(request, response);
        }
        
        if(request.getParameter("cmt")!=null){
            cmtDAO cmttt = new cmtDAO();
            String name = request.getParameter("name");
            String cid = request.getParameter("cid");
            String cmtt = request.getParameter("cmtt");
            LocalDate currentDate = LocalDate.now();
            String date = currentDate.toString();
            cmttt.addCMT(cid,id,name,cmtt,date);
            doGet(request, response);
        }
    }
